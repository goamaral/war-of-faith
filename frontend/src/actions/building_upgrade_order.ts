...
export async function issueBuildingUpgradeOrder(coords: string, buildingId: string, level: number) {
  ...
  if (resIssueBuildingUpgradeOrder.isErr()) {
    alert(`[STATE] Failed to issue building upgrade order (buildingId: ${buildingId}, level: ${level}): ${resIssueBuildingUpgradeOrder.error}`)
    return
  }
  ...
  if (resServerIssueBuildingUpgradeOrder.isErr()) {
    alert(`[SERVER] Failed to issue building upgrade order (buildingId: ${buildingId}, level: ${level}): ${resServerIssueBuildingUpgradeOrder.error}`)
...
export async function cancelBuildingUpgradeOrder(coords: string, buildingId: string, level: number) {
  ...
  if (resCancelBuildingUpgradeOrder.isErr()) {
    alert(`[STATE] Failed to cancel building upgrade order (buildingId: ${buildingId}, level: ${level}): ${resCancelBuildingUpgradeOrder.error}`)
    return
  }
  ...
  if (resServerCancelBuildingUpgradeOrder.isErr()) {
    alert(`[SERVER] Failed to cancel building upgrade order (buildingId: ${buildingId}, level: ${level}): ${resServerCancelBuildingUpgradeOrder.error}`)
