...
export async function issueTrainingOrder(coords: string, troopId: string, quantity: number) {
  ...
  if (resIssueTrainingOrder.isErr()) {
    alert(`[STATE] Failed to issue training order (troopId: ${troopId}, quantity: ${quantity}): ${resIssueTrainingOrder.error}`)
    return
  }
  ...
  if (resServerIssueTrainingOrder.isErr()) {
    alert(`[SERVER] Failed to issue troop training order (troopId: ${troopId}, quantity: ${quantity}): ${resServerIssueTrainingOrder.error}`)
...
export async function cancelTrainingOrder(coords: string, order: serverV1.Village_TrainingOrder) {
  ...
  if (resCancelTrainingOrder.isErr()) {
    alert(`[STATE] Failed to cancel training order (coords: ${coords}, orderId: ${order.id}): ${resCancelTrainingOrder.error}`)
    return
  }
  ...
  if (resServerIssueTrainingOrder.isErr()) {
    alert(`[SERVER] Failed to cancel training order (coords: ${coords}, orderId: ${order.id}): ${resServerIssueTrainingOrder.error}`)
